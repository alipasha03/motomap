...
def _build_graph_for_cc_grade_penalty():
    ...
    graph.add_edge(
        1, 2, 0, length=1200, maxspeed=60, toll="no", grade=0.11, highway="secondary"
    )
    graph.add_edge(
        2, 4, 0, length=1200, maxspeed=60, toll="no", grade=0.11, highway="secondary"
    )

    # Longer but mild slope option
    graph.add_edge(
        1, 3, 0, length=1400, maxspeed=60, toll="no", grade=0.02, highway="secondary"
    )
    graph.add_edge(
        3, 4, 0, length=1400, maxspeed=60, toll="no", grade=0.02, highway="secondary"
    )
    return graph
...
def _build_graph_for_istanbul_ferry_choice():
    graph = nx.MultiDiGraph()
    for n, (x, y) in {
        1: (29.06, 40.99),  # Kadikoy side
        2: (29.09, 41.02),  # bridge approach
        3: (28.98, 41.03),  # Besiktas side
        4: (29.03, 40.99),  # ferry pier east
        5: (29.00, 41.01),  # ferry pier west
    }.items():
        graph.add_node(n, x=x, y=y)

    # Bridge route: longer despite higher-speed motorway section.
    ...
    # Ferry route: short access roads plus explicit crossing duration.
